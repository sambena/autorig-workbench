...
def propose_tuning_candidate(spec, audit_result, iteration=0, history=None):
    ...
    # read once: the bleed and second-round strategies below the kind branches use these for every kind
    ...
    if not is_humanoid:
        ...
        if bone_info and "joint_nudge" not in tried_params:
            ...
            # the spec's points are fractions of the model's box, as the audit's at_bbox is; `at` is world units
            centroid = calculate_tear_centroid(diag["tear_sites"], worst_bone, use_bbox=True)
